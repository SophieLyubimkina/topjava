...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, Integer> dayOfYearMeals = new HashMap<>();
        List<UserMealWithExcess> result = new ArrayList<>();
        for (UserMeal meal : meals) {
            dayOfYearMeals.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), Integer::sum);
        }
        for (UserMeal meal : meals) {
            if (meal.getDateTime().toLocalTime().isBefore(endTime) && !meal.getDateTime().toLocalTime().isBefore(startTime)) {
                System.out.println(dayOfYearMeals.get(meal.getDateTime().toLocalDate()));
                boolean ecseed = dayOfYearMeals.get(meal.getDateTime().toLocalDate()) > caloriesPerDay;
                result.add(new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(), ecseed));
            }
        }
        return result;
    }
